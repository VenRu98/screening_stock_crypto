...
class ImapProcessing:
    ...
    def relogin_to_email_then_get_data(self,str_request_data, str_subject):
        while True:
            ...
            try:
                _, self.selected_mails = self.mail.search(None, '(FROM "noreply@tradingview.com" UNSEEN SUBJECT "Screener Alert: {}")'.format(str_subject) )
        ...
        for num in messages:
            ...
            for part in email_message.walk():
                if part.get_content_type()=="text/plain" or part.get_content_type()=="text/html":
                    ...
                    elements = soup.find_all('a', attrs={'style': 'text-decoration: none; color: #2962FF; word-break: break-word;', 'class': 'link'})
                    if(condition == "stock"):
                        elements_filter = [result.text for result in elements[:-2]]
                    elif(condition == "crypto"):
                        elements_filter = [result.text for result in elements[:-2] if "USDTPERP" in result.text.upper() ]
    ...
    def stock_signal_debug(self, arr):
        ...
        filtering = FilterProcessingStock()
        [dataset_signal, irisan] = filtering.process_data(dataset_signal,"long")
        if( len(dataset_signal) > 0  ):
            title = "Potensi Saham Bursa Berikutnya (Ada {} Saham):\n[TICKER | CHANGE | VOLUME | CLOSE]".format( len(dataset_signal) )
            messages = self.processing_message(dataset_signal, title)
            self.send_to_telegram_stock(messages)
    ...
    def short_crypto_signal(self):
        ...
        filtering = FilterProcessingCrypto()
        [dataset_signal, irisan] = filtering.process_data(self.dataset,"short")
        if( len(dataset_signal) > 0  ):
    ...
    def short_crypto_signal_debug(self, arr):
        ...
        filtering = FilterProcessingCrypto()
        [dataset_signal, irisan] = filtering.process_data(dataset_signal,"short")
        if( len(dataset_signal) > 0  ):
            title = "Potensi Crpto Turun 1 Jam Mendatang [Short] (Ada {} Crypto) :\n[TICKER | CHANGE]".format( len(dataset_signal) )
            messages = self.processing_message(dataset_signal, title)
            self.send_to_telegram_crypto(messages)
